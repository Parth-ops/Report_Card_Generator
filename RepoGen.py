...
for stds in dfs:
    ...
    tab1 = ax1.table(cellText=[[fname, lname, fullname, gender, dob, city, cnt]],
                     colLabels=["First Name", "Last Name", "Full Name", "Gender", "Date of birth", "City of residence",
                                "Country of residence"], loc='center', cellLoc='center')
    ...
    print("Exam Details: ")
    tab2 = ax2.table(cellText=[[regno, dot, sname]],
                     colLabels=["Registration No.", "Date and time of exam", "School Name"], loc='center',
                     cellLoc='center')
    ...
    im1 = plt.imread(dirname + '\Pics_for_assignment\{}_{}.jpg'.format(fname, lname))
    newax1 = fig1.add_axes([0.8, 0.7, 0.16, 0.15], anchor='NE', zorder=1)
    newax1.imshow(im1)
    newax1.axis('off')

    im2 = plt.imread(dirname + '\Pics_for_assignment\sci.jpg')
    newax2 = fig1.add_axes([0.1, 0.7, 0.16, 0.15], anchor='NE', zorder=1)
    newax2.imshow(im2)
    newax2.axis('off')

    ax4.axis('off')
    ax4.axis('tight')
    tab4 = ax4.table(cellText=[[total, score, result]], colLabels=["Max Marks", "Your Score", "Final Result"],
                     loc='center', cellLoc='center')
    tab4.auto_set_font_size(False)
    tab4.auto_set_column_width(col=list(range(3)))
    ...
    ax6.pie([cor, incor, unat], explode=explode, labels=labels, colors=[colours[key] for key in labels],
            autopct='%1.1f%%',
